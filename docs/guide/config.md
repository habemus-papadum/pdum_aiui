...
| `sidecars.code`         | Host the [code reader](./code-reader) sidecar. Unset = auto (on when the project has an LSP setup or well-known languages); `true` forces it, `false` disables it. Per-launch flags win: `--aiui-sidecar` / `--aiui-no-sidecar code`. |
