...
function evictedWorkspaceYaml(evicting) {
  ...
  return (
    ...
    `packages:\n${evicting.map((n) => `  - "${n}"\n`).join("")}\n` +
    [block("overrides"), block("allowBuilds")].filter(Boolean).join("\n")
  );
}
...
function reparent(dest) {
  ...
  if (existsSync(tsconfig)) {
    writeFileSync(
      tsconfig,
      readFileSync(tsconfig, "utf8").replace(/\.\.\/\.\.\/tsconfig/g, "../tsconfig"),
    );
  }
...
function produce(evicting, index, out) {
  ...
  for (const f of ["tsconfig.base.json", "biome.json"]) {
  ...
  console.log(`Wrote ${evicting.length} package(s) to ${out}`);
  console.log("Next: cd there, `pnpm install`, and run each package's own gates.");
}
