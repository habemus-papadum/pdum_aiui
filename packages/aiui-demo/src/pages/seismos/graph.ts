 ...
 */
import { agentToolkit, type Cell, cell, cellGraph, durable } from "@habemus-papadum/aiui-viz";
...
const graphBox = durable("seismos:graphBox", () => {
  const [get, set] = createSignal<{ graph: SeismosGraph; dispose: () => void }>();
  return { get, set };
});

export const seismosGraph = (): SeismosGraph | undefined => graphBox.get()?.graph;

function build(): { graph: SeismosGraph; dispose: () => void } {
  return cellGraph(() => {
...
function registerTools(): void {
  const { registerTool, registerReporter } = agentToolkit("seismos");
  ...
  registerTool({
    name: "locate",
    description:
      "Map DOM elements to source locations (data-source-loc stamps). Combine with window.__AIUI__.sourceRoot for absolute paths.",
    params: { selector: "CSS selector; first 20 matches" },
    run: (args) => {
...
if (import.meta.hot) {
  import.meta.hot.accept(() => {
    console.info("[seismos:hmr] graph rebuilt over durable DuckDB + coordinator + selection");
  });
}
