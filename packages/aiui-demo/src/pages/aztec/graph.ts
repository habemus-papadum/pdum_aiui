...
const graphBox = durable("aztec:graphBox", () => {
  const [get, set] = createSignal<{ graph: AztecGraph; dispose: () => void }>();
  return { get, set };
});

export const aztecGraph = (): AztecGraph | undefined => graphBox.get()?.graph;

function emitEveryFor(target: number): number {
  return Math.max(1, Math.ceil(target / (MAX_FRAMES - 8)));
}

function build(): { graph: AztecGraph; dispose: () => void } {
  return cellGraph(() => {
...
function registerTools(): void {
  const { registerTool, registerReporter } = agentToolkit("aztec");
  ...
  registerReporter("running", () => aztecGraph()?.shuffle.loading() ?? false);
...
if (import.meta.hot) {
  import.meta.hot.accept(() => {
    console.info("[aztec:hmr] graph module reloaded — cells rebuilt over durable roots");
  });
}
