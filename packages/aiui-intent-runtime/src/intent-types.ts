 ...
 */
import type { Ack, AttachmentChunk, AudioChunk, JsonChunk, ServerMessage } from "./protocol";

export interface IntentThread {
  /** Send the final payload (`fin`) and release the connection. */
  finish(payload?: unknown): Promise<Ack>;
  /**
   * Send a tagged JSON chunk (an `events` batch) — the `intent-v1` streaming
   * form. `fin` marks the thread's final frame.
   */
