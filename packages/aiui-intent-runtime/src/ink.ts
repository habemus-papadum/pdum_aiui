/**
 * The pen — an ADAPTER over `@habemus-papadum/aiui-ink`'s `InkSurface` (the
 * surface, the warp fade curve, and the remote feed live in that package;
 * this class keeps the composer-facing API). The policies under test are
 * unchanged:
 *
 ...
 * the host's injected styles (the surface's inline defaults agree).
 */
...
export class Ink {
  ...
  constructor(opts: {
    ...
    // The host places the canvas itself (layers.append(ink.canvas)) —
  ...
  /** How many committed strokes are on the canvas (the host's report). */
  strokeCount(): number {
