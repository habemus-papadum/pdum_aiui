/**
 * The intent turn's **thread socket half** — framework-free plumbing the host
...
export function createWire(deps: WireDeps): Wire {
  ...
  async function sendControl(
    control: "linter" | "lint",
    value: LinterVendor | LintTurnAction,
  ): Promise<void> {
    const thread = await getThread();
    if (!thread) {
  ...
  function handleServerMessage(msg: { kind: string; [key: string]: unknown }): void {
    if (msg.kind === "lowered" && Array.isArray(msg.events)) {
      ...
      // (`lint_N`) plays whenever the linter is on — `audioBack` is the
      // TTS-ack knob and must not mute it (the silent-linter bug). Everything
