 ...
 *
 * While the user is sharing, the host draws the display-capture stream to a
 ...
 * plain Node with a fake capture (jsdom has no real canvas), and the host
 * owns the canvas plumbing next to its capture tool.
 */
 ...
 * asks "did a frame come back?". The host passes `ShotPixels` (thumb + JPEG
 ...
 * counting from 0 (a new share resets it — the host bumps the share
