 ...
 * `kind === "lowered-prompt"`. The wire itself deliberately ignores it — see
 * the handler in wire.ts — a host that wants the result observes it on the
 * raw socket (the `onSocket` hook); the type exists for exactly that.
 */
