...
| `@habemus-papadum/aiui-viz/testing` → `cellHarness`, `whenReady`, `whenState`, `recordCommits`, `tick` | The cell-testing harness: build a graph under a disposable owner (kept live), await states/values with diagnostic timeouts, record a stream's commits. Buries the Solid trivia (owners, write batching, tick counting) so a dataflow test reads as intent — and makes the per-input probe that catches an undeclared dependency three lines. |
