...
| `control`, `action`, `controlSurface`, `actionByName`, `subscribeControlSurface` | The **control surface**: `control({ value, min?, max?, step?, unit?, options?, … })` declares a curated, durable, validated knob (any `T`; clamp/snap for numbers, enum check for `options`); `action({ name, run })` registers a verb. Names, definition sites, and descriptions are injected by the aiui compiler (the doc comment IS the description). Declaring is exposing — see `registerStandardTools`. |
| `scope` | Instance identity for **reusable slices**: `scope("left")` threads through a slice factory's declarations (`control({ scope, … })`, `cell(…, { scope })`, `action({ scope, … })`, `s.durableSignal(…)`) so two instances of one factory get qualified names (`left/freq`), distinct durable state, distinct derived tools, and instance-correct edges. Explicit by design — no ambient "current scope". |
| `registerStandardTools` | The agent tools every aiui app should have and none should write, derived from the declarations: `report` (brief/full — controls, cells with state, actions, and the live control→cell dependency edges), `set` (validated write, returns what was written), `locate` (CSS selector → attribution stamps), and **one real named tool per registered action**. |
| `ControlSlider`, `ControlToggle`, `ControlScrub`, `ControlSelect` | Control-bound widgets: bounds, step, unit, and options come from the control's declaration (never re-typed in JSX), the root carries the `data-control` stamp, description becomes the tooltip. `ControlScrub` is the number-in-the-prose pill (press and drag sideways; a fixed drag length crosses the declared range, `log` for decades); `ControlSelect` binds an enum control's `options` to a `<select>` (writes the declared option, not its string). Porcelain by extraction — hand-roll bindings the shipped widgets don't cover. |
