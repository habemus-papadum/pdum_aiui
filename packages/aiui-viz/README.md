...
| `ControlSlider`, `ControlToggle` | Control-bound widgets: bounds, step, and unit come from the control's declaration (never re-typed in JSX), the label carries the `data-control` stamp, description becomes the tooltip. Porcelain by extraction — hand-roll bindings the shipped widgets don't cover. |
