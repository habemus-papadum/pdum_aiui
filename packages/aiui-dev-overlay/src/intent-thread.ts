 ...
 * pushes. Extracted from the overlay host's `openThread` (intent.ts, which
 * now calls this) so the browser extension's panel — a different host of the
 * same wire (`multimodal/shell/wire.ts`) — opens threads through ONE
 * implementation instead of a hand-rolled twin (Phase C1 of the extension
 * plan). Everything host-specific stays with the host: meta collection (page
 * instrumentation vs tab identity), connect-failure surfacing, and any extra
 * raw-socket listeners ride the `onSocket` hook.
 */
