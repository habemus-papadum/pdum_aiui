 ...
 * thread's verbs over the channel socket) and `OpenThreadOptions`. Extracted
 * from intent.ts (Phase C1) so wire-level consumers — the shared wire shell
 * and the browser extension's panel host — can import the types without
 * pulling the overlay host's whole module graph into their TypeScript
 * programs (a type-only import still typechecks the imported file; intent.ts
 * drags the multimodal world). intent.ts re-exports these, so existing
