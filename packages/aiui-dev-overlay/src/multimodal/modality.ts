...
import { loadIntentOverrides, mountAdvancedConfig } from "./advanced-config";
...
export function multimodalModality(
  configOverride: Partial<IntentPipelineConfig> = {},
): IntentModality {
  const config: IntentPipelineConfig = { ...DEFAULT_INTENT_CONFIG, ...configOverride };

  return {
    format: "intent-v1",
    label: "Multimodal",
    mount(container: HTMLElement, ctx: IntentToolContext) {
      // Config layers: DEFAULT ← Vite intent option (the closure's `config`,
      // already merged) ← persisted panel overrides. `base` is the first two,
      // kept for the advanced panel's delta; `config` becomes the effective one
      // (mutated in place so dynamic reads + the next hello pick up live edits).
      const base: IntentPipelineConfig = { ...config };
      Object.assign(config, loadIntentOverrides());
      ...
      function openThreadSocket(): void {
        ...
        threadPromise.catch((error) => rememberError(error));
      }
      ...
      const noteCorrectionFailure = (error: unknown): void => {
        ...
        engine.events.push({
          at: Date.now(),
          type: "note",
          text: `correction pipeline failed (${config.corrector}): ${
      ...
      // ── talk plumbing (mock local / channel upload) ──────────────────────────
      async function talkStart(): Promise<void> {
        ...
        // the default transcriber even if the mic is unprompted or denied. Read
        // dynamically, so an advanced-config switch to "openai" applies next talk.
      ...
      async function talkEnd(): Promise<void> {
        ...
        if (!thread) {
          ...
          ctx.setStatus("transcription needs the channel — none connected");
          return;
        }
      ...
      mountAdvancedConfig(container, { base, effective: config, onApply: applyEffective });
