/**
 * The app-under-test: the familiar "spectra" viewer, annotated the way the
 * overlay's source-locator Vite plugin annotates a real app — `data-cell`
 * (dataflow node name) + `data-source-loc` (file:line:col, app-root-relative).
 * Those are exactly the stamps the graduated shot locator (`locateComponents`)
 * and the selection watcher resolve against; here they're hand-written to keep
 * the lab working, in production they come from build-time instrumentation.
 * (The lab has no `sourceRoot`, so the locator passes the relative stamp
 * through — a real app resolves it to an absolute path.)
 */
export function mountScenery(host: HTMLElement): void {
  host.innerHTML = `
    <div data-cell="AppShell" data-source-loc="workbench/src/scenery.ts:14:5">
      <header data-cell="Header" data-source-loc="workbench/src/scenery.ts:16:7">
        <h1><span>spectra</span> · absorption viewer</h1>
        <div class="sub">workbench scenery — arm the overlay (\`), then talk / draw / shoot</div>
      </header>
      <main>
        <div class="card" data-cell="SpectrumPlot" data-source-loc="workbench/src/scenery.ts:22:9">
          ...
          <div class="legend" data-cell="Legend" data-source-loc="workbench/src/scenery.ts:35:11">
            <span><i style="background:#8ab4f8"></i>sample A-113</span>
            <span><i style="background:#7ee0a3"></i>baseline</span>
          </div>
        </div>
        <div class="card" data-cell="SampleTable" data-source-loc="workbench/src/scenery.ts:41:9">
          <table>
