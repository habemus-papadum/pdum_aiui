 ...
 * `http://<address>:<channelPort>/pencil/` — the open question is which
 * addresses can reach it. This command finds every running channel via the
 * on-disk registry, confirms the pencil sidecar answers `/pencil/info`, and
 * reads the channel's bind from `/health`: a host-bound channel gets its LAN
 * URL(s) printed (open one on the iPad — macOS Universal Clipboard pastes
 * straight across); a loopback-bound channel gets the loopback URL plus a
 * reminder that reaching it from the iPad is a tunnel of the user's own
 * making (Tailscale, `ssh -L`).
 *
 * A channel without the pencil sidecar simply doesn't answer `/pencil/info`.
 *
