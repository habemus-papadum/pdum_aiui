...
describe("TracesPane with a pinned session (the ?session= deep link)", () => {
  it("filters the list to the pinned label, not the answering server's own", async () => {
    ...
    const rows = [...pane.root.querySelectorAll("button.aiui-dbgt-row")];
    expect(rows).toHaveLength(1);
    // The pinned session's row carries no meta badge (it IS the current
    // session); the server's own would have been badged under "all sessions".
    expect(rows[0].querySelector(".aiui-dbgt-badge.meta")).toBeNull();
