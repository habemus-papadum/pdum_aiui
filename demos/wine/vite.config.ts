...
export default defineConfig({
  ...
  optimizeDeps: {
    ...
    // pre-bundling relocates the module into .vite/deps/ where those worker
    // files don't exist, so cluster-label generation hangs forever at
    // "Generating labels...". Excluded, the package serves from node_modules
    // and the relative worker URLs resolve. (This is a node_modules dep —
    // the never-optimizeDeps.include rule is about WORKSPACE packages.)
